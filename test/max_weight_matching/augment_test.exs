defmodule MaxWeightMatching.AugmentTest do
  ...
  describe "augment_matching/2" do
    ...
    test "preserves matching for vertices not on path" do
      ...
      ctx =
        graph
        |> Context.new()
        |> then(&%{&1 | vertex_mate: %{0 => -1, 1 => 2, 2 => 1, 3 => -1, 4 => 5, 5 => 4}})
        |> Augment.augment_matching(path)
  ...
  describe "augment_blossom/3" do
    ...
    test "rotates subblossom list to make sub the new base" do
      ...
      ctx =
        ctx
        |> Context.add_blossom(blossom)
        |> Context.update_blossom(sub0_id, parent_id: blossom.id)
        |> Context.update_blossom(sub1_id, parent_id: blossom.id)
        |> Context.update_blossom(sub2_id, parent_id: blossom.id)
        |> Context.set_vertices_blossom([0, 1, 2], blossom.id)
        |> Augment.augment_blossom(blossom.id, sub2_id)
    ...
    test "updates vertex_mate for internal edges" do
      ...
      ctx =
        graph
        |> Context.new()
        |> then(&%{&1 | vertex_mate: %{0 => 1, 1 => 0, 2 => -1}})
    ...
    test "handles augmentation from position 1 (odd position)" do
      ...
      ctx =
        ...
        |> Context.update_blossom(sub2_id, parent_id: blossom.id)
        |> Context.set_vertices_blossom([0, 1, 2], blossom.id)
        |> Augment.augment_blossom(blossom.id, sub1_id)
    ...
    test "raises when sub_id not found in blossom" do
      ...
      ctx =
        ...
        |> Context.update_blossom(sub1_id, parent_id: blossom.id)
        |> Context.update_blossom(sub2_id, parent_id: blossom.id)
        |> Context.update_blossom(sub3_id, parent_id: blossom.id)

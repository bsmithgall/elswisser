defmodule ElswisserWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: ElswisserWeb.Gettext
  ...
  slot(:icons, doc: "slot for all inbound icons")

  def icon_button_row(assigns) do
    ~H"""
  ...
  attr(:rounded_hover, :boolean, default: true)
      ...
      <table class="w-[40rem] mt-4 sm:w-full">
        ...
        >
          <tr
            ...
            class={["group hover:bg-zinc-50", @striped && "even:bg-zinc-100"]}
          >
            <td
              :for={col <- @col}
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-1 text-right text-sm font-medium">
                <span
                  :if={@rounded_hover}
                  class="absolute -inset-y-px -right-4 left-0 group-hover:bg-zinc-50 sm:rounded-r-xl"
                />
                <span
                  :for={action <- @action}
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ElswisserWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ElswisserWeb.Gettext, "errors", msg, opts)
    end
  end

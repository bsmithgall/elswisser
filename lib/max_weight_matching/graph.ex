defmodule MaxWeightMatching.Graph do
  ...
  defstruct [:edges, :num_vertex, :num_edges, :adjacent_edges, :integer_weights]

  @doc """
  Gets an edge by index. O(1) operation.
  """
  @spec get_edge(t(), non_neg_integer()) ::
          {non_neg_integer(), non_neg_integer(), number()}
  def get_edge(%__MODULE__{edges: edges}, index) do
    Map.fetch!(edges, index)
  end
  ...
  def new(edges) when is_list(edges) do
    ...
    # Convert edge list to map for O(1) index access
    edges_map =
      edges
      |> Enum.with_index()
      |> Map.new(fn {edge, idx} -> {idx, edge} end)
